...
const yahoo = async (url) => {
  ...
  const cookieButtonInnerText = '.btn.primary';
  ...
  // Close cookie popup
  await page.click(cookieButtonInnerText);
  await page.waitFor(3000);

  // Hide ad at page top
  await page.evaluate(() => {
    let topAd = document.querySelector('#mrt-node-Lead-0-Ad');
    topAd.style.display = 'none';
  });

  // Extract headline
  const headline = await page.evaluate(() => {
    let headline = document.querySelector('.Mt(0)').innerText;
    return headline;
  });
